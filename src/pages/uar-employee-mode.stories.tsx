...
import {
  UserAccessDetailsSheet,
  type UserAccessDetailsSheetData,
} from '@/components/ui/user-access-details-sheet';
...
import {
  ...
  type ApplicationRecommendationsTab,
} from '@/components/ui/application-recommendations-card';
...
// Mock data for the User Access Details sheet
function getSheetDataForRow(rowId: string): UserAccessDetailsSheetData {
  const userIndex = Math.max(0, parseInt(rowId.replace('row-', '') || '1', 10) - 1);
  const user = sampleUsers[userIndex % sampleUsers.length] || sampleUsers[0];
  const riskLevels: Array<'Low' | 'Medium' | 'High'> = ['Low', 'Medium', 'High'];
  const riskLevel = riskLevels[userIndex % 3];
  const insights = generateInsightsForRow(userIndex, 1, true);
  const suggestedAction = getRecommendedAction(insights, riskLevel);
  const roles = ['HR Partner', 'Engineering Lead', 'Finance Analyst', 'Product Manager'];
  const departments = ['HR', 'Engineering', 'Finance', 'Product'];
  const apps: Array<{ name: string; userType: string; lastActivity: string }> = [
    { name: 'Jira', userType: 'Jira User', lastActivity: 'Last login: 150 days ago' },
    { name: 'Slack', userType: 'Member', lastActivity: 'Last active: 7 days ago' },
    { name: 'Google Workspace', userType: 'User', lastActivity: 'Last login: 2 days ago' },
    { name: 'AWS PROD', userType: 'ViewOnly Assume Role', lastActivity: 'Last login: 30 days ago' },
    { name: 'GitHub', userType: 'Developer', lastActivity: 'Last active: 1 day ago' },
    { name: 'Salesforce', userType: 'Standard User', lastActivity: 'Last login: 5 days ago' },
    { name: 'Okta', userType: 'User', lastActivity: 'Last login: 1 day ago' },
    { name: 'Confluence', userType: 'Editor', lastActivity: 'Last active: 14 days ago' },
  ];
  // Vary count: some users have 1–3 apps, some have more (up to 8) to exercise the table
  const accessCount = userIndex % 5 === 0 ? Math.min(8, 2 + (userIndex % 6)) : 1 + (userIndex % 3);
  return {
    ...
    accessDetails: apps.slice(0, accessCount),
    insight: insights[0],
    suggestedAction,
    riskLevel,
    reviewHistory: getReviewHistoryForRow(rowId),
  };
}
  ...
  render: () => {
    ...
    return (
      ...
      <UAREmployeeModeV12
        ...
        showSignOffButton={true}
        signOffProgressCompleted={250}
        signOffProgressTotal={500}
      ...
      <Dialog open={modifyDialogOpen} onOpenChange={(open) => {
        ...
        <UserAccessDetailsSheet
          ...
          rowId={reviewHistoryPanelRowId}
          getSheetDataForRow={getSheetDataForRow}
          onCertify={(rowId) => {
            const data = getSheetDataForRow(rowId);
            handleCertifyClick({ id: rowId, riskLevel: data.riskLevel });
          }}
          onRevoke={(rowId) => {
            const data = getSheetDataForRow(rowId);
            handleRevokeClick({ id: rowId, riskLevel: data.riskLevel });
          }}
          onModify={(rowId) => {
            const data = getSheetDataForRow(rowId);
            handleModifyClick({ id: rowId, riskLevel: data.riskLevel });
          }}
        />
        <Toaster />
      </>
...
export const RecordOverviewV13Story: Story = {
  name: 'Record Overview 1.3',
  render: () => {
    const [selectedRows, setSelectedRows] = useState<Set<string>>(new Set());
    // Pre-populate ~70% of rows (35 out of 50) with actions for demonstration
    // This represents rows that have been actioned upon
      ...
      <UAREmployeeModeV12
        ...
        overviewCard={
          <>
            ...
            <OverviewCard
              totalCount={50}
              certifiedCount={certifiedRows.size}
              revokedCount={revokedRows.size}
            ...
            />
            {recommendationsActiveTab !== 'manual-review' && (
              <RecommendedActionBanner
                recommendation={
                  recommendationsActiveTab === 'certify'
                    ? 'Certify'
                    : recommendationsActiveTab === 'revoke'
                      ? 'Revoke'
                      : 'Modify'
                }
                recordCount={
          ...
          return (
            <div className="flex items-center gap-3 min-w-0">
              ...
              <button
                ...
                className="flex items-center gap-3 min-w-0 flex-1 rounded-md hover:bg-muted/60 active:bg-muted text-left transition-colors cursor-pointer -ml-1 pl-1 py-0.5"
